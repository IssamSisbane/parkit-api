...
class PlaceService {
    ...
    public async createPlace(placeData: IPlace): Promise<IPlace> {
        ...
        const findPlace: IPlace | null = await this.places.findOne({ numero: placeData.numero });
        if (findPlace) throw new HttpException(409, `This numero ${placeData.numero} already exists`);
    ...
    public async updatePlace(numero: Number, placeData: IPlace): Promise<IPlace> {
        ...
        if (placeData.numero) {
            const findPlace: IPlace | null = await this.places.findOne({ numero: placeData.numero });
            if (findPlace && findPlace.numero != numero) throw new HttpException(409, `This numero ${placeData.numero} already exists`);
        }

        const updatePlaceByNumero: IPlace | null = await this.places.findByIdAndUpdate({ ...placeData });
